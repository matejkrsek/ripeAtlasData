...
# Začátek za 1 minutu, konec po 24 hodinách
...
# === Flatten all probes ===
all_probes = [p for region in regions.values() for p in region]
...
for target in targets:
    measurement_data = {
        "definitions": [
            {
                "target": target,
                "description": f"FINAL TRACE to {target} every 15 min for {duration_hours}h",
                "type": "traceroute",
                "af": 4,
                "packets": packets,
                "protocol": "ICMP",
